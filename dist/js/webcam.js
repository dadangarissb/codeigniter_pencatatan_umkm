...
window.webcam = {
	...
	set_url_action: function(url_action){
		this.url_action = url_action;
	},
	
	click: function(){
		var NIK = $("#nik").val();
		this.url_action = this.url_action+"?nik="+NIK;
		if(NIK!=''){
			this.get_movie();
			if(this.cam){
				this.api_url=this.api_url+"?nik="+NIK+"&cam="+this.cam;
				this.snap();
			}else{
				this.api_url=this.api_url+"?nik="+NIK+"&cam="+this.cam;
				$.get(this.api_url,window.location.href=this.url_action);
			}
		}else{
			$.get(this.api_url,window.location.href=this.url_action);
		}
	...
	set_baseurl_swf: function(url_swf){
		this.swf_url = url_swf;
	},
	
	set_baseurl_shutter: function(url_shutter){
		this.shutter_url = url_shutter;
	},
	...
	get_movie: function() {
		...
		if (!this.loaded) return this.cam=false;//alert("ERROR: Movie is not loaded yet");
